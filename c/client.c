...
int fetch_remaining_blocks_http(struct zsync_state *z, const char *url,
...

    {
        ...
        do {
            ret = range_fetch_perform(rf, &prog);
	...
	curl_off_t ultotal, curl_off_t ulnow)
{
	...
	struct progress *progress = myp->p;
	struct zsync_state *z = myp->z;

	do_progress(progress, calc_zsync_progress(z), dlnow);
	...
	curl_off_t ultotal, curl_off_t ulnow)
{
	...
	do_progress(progress, dltotal ? (100.0 * dlnow / dltotal) : 0, dlnow);
